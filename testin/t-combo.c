/*
 * t-combo  dthread test for some combo ops
 * 20-Jun-2026  chuck@ece.cmu.edu
 */
...
int app_main(int argc, char **argv) {
    ...
    printf("app_main: testing join of detached child thread\n");
    ...
    if (rv != 0) {
        ...
        printf("app_main: created!  sleep 3\n");
    }

    sleep(3);

    printf("app_main: detach child\n");
    rv = dthread_detach(child);
    if (rv != 0) {
        printf("app_main: detach failed! (%s)\n", strerror(rv));
        errcnt++;
    } else {
        printf("app_main: child detach.  going to join!\n");
    }

    sleep(1);

    printf("app_main: join detached child\n");
    rv = dthread_njoin(child, &ret);
    if (rv != EINVAL) {
        printf("app_main: join return unexpected rv! (%s)\n", strerror(rv));
        errcnt++;
    } else {
        printf("app_main: detached child join returned correct einval.\n");
    }

    sleep(1);

    printf("app_main: cancel detached child (should still work)\n");
    rv = dthread_cancel(child);
    if (rv) {
        printf("app_main: cancel failed!  (%s)\n", strerror(rv));
        errcnt++;
    } else {
        printf("app_main: canceled ok!\n");
    }

    sleep(3);

    printf("app_main: testing detach interrupting pending join\n");

    arg.dt_argret_type = DTHREAD_NODATA;
    rv = dthread_ncreate(&child, NULL, rem_loop2, &arg);
    if (rv != 0) {
        printf("app_main: ncreate failed! (%s)\n", strerror(rv));
        errcnt++;
    } else {
    ...
    printf("app_main: join child before it detaches\n");
    rv = dthread_njoin(child, &ret);
    if (rv != EINVAL) {
        printf("app_main: join return unexpected rv! (%s)\n", strerror(rv));
        errcnt++;
    ...
    printf("app_main: manually waiting for child to finish\n");
    sleep(14);
...
dthread_argret_t rem_loop2(dthread_argret_t *dt_arg) {
    ...
    printf("rem_loop2: running\n");
    for (lcv = 0 ; lcv < 10 ; lcv++) {
        printf("rem_loop2: tick %d...\n", lcv);
        if (lcv == 4) {
            printf("rem_loop2: detaching self\n");
